 ...
 *
 * Modified for SoundTracker 1998 by Michael Krause
 *
...
static int
gtk_spin_button_find_display_digits (GtkAdjustment *adjustment)
{
  int num_digits;
			      ...
			      GtkRequisition *requisition)
{
  ...
  requisition->width = MAX (MIN_SPIN_BUTTON_WIDTH,
			    gtk_spin_button_find_display_digits(GTK_SPIN_BUTTON(widget)->adjustment)
			     ...
			     gint        *position)
{
  ...
  if (spin->numeric)
    {
#if ((GTK_MAJOR_VERSION * 0x10000) + (GTK_MINOR_VERSION * 0x100) + GTK_MICRO_VERSION) >= 0x010107
      struct lconv *lc;
      gboolean sign;
      gint dotpos = -1;
      gint i;
			   ...
			   guint           digits)
{
  ...
  gtk_spin_button_set_adjustment (spin_button, adjustment);
  spin_button->digits = digits;
  sprintf (buf, "%0.*f", (int)digits, adjustment->value);
  gtk_entry_set_text (GTK_ENTRY (spin_button), buf);
  spin_button->climb_rate = climb_rate;
#endif
}
				...
				GtkAdjustment *adjustment)
{
  ...
  if (spin_button->adjustment != adjustment)
	...
	if(spin_button->adjustment == NULL
	   || (adjustment != NULL && gtk_spin_button_find_display_digits(adjustment) > gtk_spin_button_find_display_digits(spin_button->adjustment)))
	    do_resize = TRUE;
